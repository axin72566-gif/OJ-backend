...
本质上是通过 Http 协议、或其他网络协议进行通讯来实现。

![image-20251027204937572](./OJ.assets/image-20251027204937572.png)

主要包括：网关、限流组件、消息队列、注册中心、分布式事务、服务远程调用

### 微服务划分

**依赖服务：**一般就是 Spring Cloud Alibaba 的相关组件

- Nacos：负责服务的注册和配置的管理
- Gateway：网关入口，聚合所有的接口，统一处理前端的所有请求

**公共服务：**

- 公共模块 common：全局异常处理器、请求响应封装类、公共的工具类
- 模型类 model：很多服务公用的实体类，比如 User，Question 之类的
- 公用接口模块：只存放接口，没有实现，一般就是 Controller 的接口
...
可以使用 SpringBoot 的 context-path 统一修改各项目的接口前缀

1. **用户服务：**

- /api/user
- /api/user/inner（这个是内部调用的，不给用户开放）

2. **题目服务：**

- /api/question（包括题目提交的接口）
- /api/question/inner
...
这里使用阿里的云原生脚手架创建项目  <https://start.aliyun.com/>

创建好之后根据模块的划分创建模块，公共模块 common、model 使用 Java 项目创建，其他使用 SpringBoot 项目创建，检查 pom 文件是不是正常，父子关系记得加上，这样子模块继承父模块的配置，避免版本冲突。
...
questionService.update(new UpdateWrapper<Question>().eq("id", questionId).setSql("acceptedNum = acceptedNum + 1"))
```
...
- @FeignClient (name = "OJ-backend-user", path = "/api/user/inner")，注解用来暴露服务，其他服务可以到注册中心中根据服务名称找服务，path 用来指定全局服务地址前缀
- 下面两个方法上的注解，用来指定要找服务模块中的哪一个服务
- 下面三个方法不加注解就当工具类来使用，不需要远程服务

（**这个远程调用是指你要调用别的 service ，比如 questionService 需要调用 userService ，这时候 userFeignClient 就派上用场了，如果说题目提交调用题目服务，就不需要调用 Client 。而且自己服务模块下不要继承 FeignClient 接口，不要搞混了，自己服务模块还是继承 service 接口**）

其他的服务使用客户端 FeignClient 调用别的服务，但是现在她还只是一个接口，并没有实现，所以现在要创建一个实现类。流程大致就是别的服务原来是调用 Service 的，现在由于服务不在一个模块中，所以要统一到集中接口的模块中寻找，而 Feign 会到注册中心中根据模块的注册名称和 Feign 定义时的路径寻找方法，从而实现调用。
...
**InnerController 记得也要写 @RequestMapping("/inner")，本质上这也是一个控制层接口，只不过只是对内供其他服务调用！！！！**

### 服务注册

现在外层模块当中引入 nacos 的依赖，然后写 yml 文件，然后在有启动类上打上注解

```java
@EnableDiscoveryClient // 让自己注册服务，而且发现其他服务
@EnableFeignClients(basePackages = "com.axin.OJbackendfeign") // 负责调用其他服务，扫描FeignClient注解的接口
```

- 第一个注解的作用是，当这个服务的启动类运行的时候，会把自己注册到 nacos 中，并且能发现别的服务
- 第二个注解的作用是，当这个服务需要调用别的服务的时候，要使用 FeignClient ，这个注解会扫描对应包中打上 @FeignCilent 注解的接口，将他们自动注册为容器的 Bean 从而调用，和之前的 Service 一样。
- 记得引入负载均衡器 LoadBalancer 。

下面就是测试了。

### 网关

网关用来统一接收前端的请求，然后转发到各个服务。

- 现在的各个服务都是分散开来的，前端不好调用。
- 可以在网关中统一做一些处理，比如限流，鉴权等等。
...
[Knife4j](https://doc.xiaominfo.com/docs/middleware-sources/spring-cloud-gateway/spring-gateway-introduction)
...
上面的是让项目支持 redis ，下面的是支持 session 存储到 redis 中。
...
这个 path 的作用是，我第一个请求用户服务登录，在 redis 里面存了一个 session ，返回了一个 sessionid 给前端，此时的 session 种的路径是根据上下文定的，也就是 context-path : /api/user，那么下一次我请求题目服务的时候，也需要 sessionid ，此时的请求上下文是 /api/question ，找不到刚才种的 cookie ，所以会报错未登录。解决方法就是统一指定一下种 session 的路径。
...
这里可以使用 JWT 来获取用户信息实现鉴权。

### 消息队列

使用 RabbitMQ 消息队列

首先生产者和消费者都引入依赖，基于 RabbitMQ 实现的模板
...
接着编写生产者和消费者，可以先写消费者再写生产者，这样消息保险不会漏掉

生产者：

```java
rabbitTemplate.convertAndSend("oj_exchange", "judge", String.valueOf(questionSubmitId));
```
...
public class MessageConsumer {
	...
	/**
	 * 接收消息
	 * @param message 消息内容，这里是题目提交记录id
	 */
	@RabbitListener(bindings = @QueueBinding(
			value = @Queue(name = "judge_queue"),
			exchange = @Exchange(name = "oj_exchange"),
			key = {"judge"}
	))
